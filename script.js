...
// ---------- Smooth Inertial Scrolling Engine (Lenis) ----------
(function initSmoothScroll() {
  if (typeof Lenis !== 'undefined') {
    const lenis = new Lenis({
      duration: 1.4,                             // Relaxed deceleration duration for slower, smoother glide
      easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)), // Silky exponential ease-out
      orientation: 'vertical',
      gestureOrientation: 'vertical',
      smoothWheel: true,
      wheelMultiplier: 0.72,                     // Moderates scroll travel speed per notch for controlled pace
      touchMultiplier: 1.1,
      infinite: false,
    });
    ...
    // Sync scroll-driven visual effects on Lenis scroll tick
    lenis.on('scroll', () => {
      if (typeof updateMarquee === 'function') updateMarquee();
      if (typeof updateAboutText === 'function') updateAboutText();
      if (typeof updateProjectScales === 'function') updateProjectScales();
    });

    // Smooth anchor navigation for all internal links (#about, #skills, #projects, #contact)
